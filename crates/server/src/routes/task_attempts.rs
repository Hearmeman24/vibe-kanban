...
pub async fn create_task_attempt(
    ...
    let workspace = match payload.mode {
        ...
        WorkspaceMode::Branch => {
            ...
            // Use executor_name if provided, otherwise use the executor from profile
            let executor_name = payload
                .executor_name
                .clone()
                .unwrap_or_else(|| executor_profile_id.executor.to_string());
            Session::create(
                pool,
                &CreateSession {
...
pub async fn force_push_task_attempt_branch(
    ...
    deployment
        .git()
        .push_to_github(&push_path, &workspace.branch, true)?;
    Ok(ResponseJson(ApiResponse::success(())))
}
...
pub async fn run_cleanup_script(
    ...
    // Branch-only workspaces don't support cleanup scripts (no container/worktree)
    if workspace.is_branch_only() {
        return Ok(ResponseJson(ApiResponse::error_with_data(
            RunScriptError::BranchOnlyModeNotSupported,
        )));
